...
给完视角之后，如果用户对某一个角度明显更感兴趣，可以说：「这个方向值得深入，/thinker 可以帮你把它想透。」如果用户的请求本身还很模糊，先建议 `/clarifier`。如果用户是在“说不清但看得出对不对”的状态，用原型/多方案帮助暴露 Unknown knowns，再建议 `/thinker` 收敛。如果角度太多、用户开始迷失，建议 `/conductor` 来收。一句话带过，不要打断节奏。
