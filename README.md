...
```bash
npx skills add simplecyon/mindforge-skills --skill '*' -g -a codex -y
```

### OpenCode / Cursor / Gemini CLI

```bash
npx skills add simplecyon/mindforge-skills --skill '*' -g -a opencode -a cursor -a gemini-cli -y
```

`-a` 只安装到你实际使用的 agent，避免 CLI 对不支持 global skill 的平台（如 PromptScript）报出无关的失败。需要其他 agent 时，把上面的 agent 名替换为对应名称即可。
